...
looper = 0
prevvalue = 1
while True: #main loop, switches the play status based on the toggle, also switches to web player automatically
    if(switch.value == 0 and prevvalue == 1):
        prevvalue = 0
        deviceid = get_devices()
        if deviceid != "0":
            transer_playback(deviceid)
        play()
    if(switch.value == 1 and prevvalue == 0):
        prevvalue = 1
        #deviceid = get_devices()
        #if deviceid != "0":
            #transer_playback(deviceid)
        pauseplay()
    if looper >= 1000000: #every 50 loops, we update the now playing file
        update_now_playing()
        looper = 0
        print(looper)
    else:
