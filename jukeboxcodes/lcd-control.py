...
while True:
    ...
    if int(lines[3]) < 15: #if we're displaying playlist, then lines 2 is the one we care about
    ...
    #this is insane, len consider \n to be ONE CHARACTER???? I SPENT TWO HOURS TRYING TO FIGURE OUT WHAT I DID WRONG
    
    if len(lines[0]) <= 17:
        for i in range(17 - len(lines[0])):
            lines[0] = lines[0].split('\n')[0] + " " + "\n" 
    if len(lines[x]) <= 17: #cuz i think they add another space to the ends
        for i in range(17 - len(lines[x])):
    ...
    if len(lines[0]) <= 17 and len(lines[x]) <= 16: #if both lines are contained by a single line, then we are chill and just print
        lcd.message = current_text #set the message
    else: 
        if len(lines[0]) > 17: #the first line is too long
            lcd.message = str(len(lines[0])) + "yes"
        ...
        lcd.message = topline + bottomline
