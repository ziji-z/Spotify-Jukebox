...
while True: #main loop
    ...
    task = input("1 for current song, 2 for to play/pause, 3 to skip, 4 to go back, 5 to exit, 6 to switch to webplayer")
    '''playing = currently_playing()
    if(switch.value == 1 and not playing['playing']):
        play()
    if(switch.value == 0 and playing['playing']):
        pause()'''
    ...
    match task:
        case "1":
            playing = currently_playing()
            if playing['playing']:
                print("Currently playing " + playing['name'])
                print("by", end=" ")
                for i in playing['artists']:
                    print(i + " ")
        case '2':
        ...
        case '5':
            sys.exit()
        case '6':
            web_player = get_devices()
            transer_playback(web_player)
