...
def get_playlists(): #get a list of playlists of the current user
    playlists = [] #empties the list first
...
playlists = get_playlists() #get the list of playlists

display_playlist(playlists[current_selection]["name"])

def incrementup(): #go forward in finding all the playlists
...
def selectplaylist(): #plays the currently selected playlist
    global current_selection
    global playlists
    play_playlist(playlists[current_selection])
    update_now_playing()
    
    playlists = get_playlists() #updates the playlist list just in case
    '''if current_selection >= len(playlists): #in case we deleted a playlist
        current_selection = 0'''

rotarybutton = Button(6,bounce_time = 0.1) # just the pressed button

rotor = RotaryEncoder(21,20)
rotarybutton.when_pressed = selectplaylist
rotor.when_rotated_clockwise = incrementup
rotor.when_rotated_counter_clockwise = incrementdown
